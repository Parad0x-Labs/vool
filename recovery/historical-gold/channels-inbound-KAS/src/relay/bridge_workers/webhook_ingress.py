...
_PROTECTED_OUTBOUND_TOPICS = frozenset({"discord_outbound", "telegram_outbound"})
...
def build_handler(forwarder: MirrorForwarder):
    class WebhookIngressHandler(BaseHTTPRequestHandler):
        ...
        def _reject_protected_snapshot(self, topic: str) -> bool:
            if topic not in _PROTECTED_OUTBOUND_TOPICS:
                return False
            self._write_json(409, {"ok": False, "error": "protected_outbound_topic"})
            return True
        ...
        def do_POST(self) -> None:
            ...
            if path.startswith("/snapshot/"):
                ...
                if not topic:
                    ...
                    return
                if self._reject_protected_snapshot(topic):
                    return
