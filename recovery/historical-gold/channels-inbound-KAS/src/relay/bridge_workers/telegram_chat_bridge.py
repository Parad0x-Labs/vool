"""Legacy Telegram <-> local VOOL chat bridge.
...
This legacy HTTP path is fail-closed until it can carry an in-process host-issued external
identity. It remains available only to return a bounded setup response, not to reach the agent.
...
- **Channel surface.** Every relayed turn posts surface="channel" and is rejected by the local API
  before session, history, model, or tool execution.
...
class TelegramChatBridge:
    ...
    def _default_vool_call(self, text: str, chat_id: str) -> str:
        ...
        req = request.Request(
            f"{_API_URL}/api/chat", data=json.dumps(payload).encode("utf-8"),
        ...
        try:
            ...
            if exc.code == 403:
                return "Remote chat is unavailable until secure external ingress is configured."
