...
from core.presentation.model import (
    Callout, Chart, Code, Heading, Link, List, MetricLine, Paragraph,
    Progress, Quote, RenderDocument, Sources, Span, Table, Timeline,
)
...
class PlainTextRenderer:
    ...
    def _table(self, t: Table) -> str:
        if self.intent.narrow() or len(t.headers) > 4:
            ...
            cards = []
            for row in t.rows:
                cards.append("\n".join(f"{h}: {c}" for h, c in zip(t.headers, row)))
            return "\n--\n".join(cards)
        widths = [max(len(t.headers[i]), *(len(r[i]) for r in t.rows))
                  for i in range(len(t.headers))]
        def fmt(cells): return "  ".join(c.ljust(w) for c, w in zip(cells, widths)).rstrip()
...
def chart_as_table(chart: Chart) -> Table:
    headers = ["Label", f"Value ({chart.series[0].unit if chart.series else ''})"]
    rows = [[p.label, f"{p.value:g}" + (f" · {p.status.name}" if p.status else "")]
            for p in chart.series]
    if chart.total is not None:
        headers.append("Total")
        rows = [r + [f"{chart.total:g}"] for r in rows]
