...
class MalformedCompartment(ValueError):
    """The compartment markup itself is broken — refuse rather than guess."""


class CompartmentLeak(RuntimeError):
...
def parse_compartments(text: str) -> tuple[str, dict[str, Compartment]]:
    ...
    while True:
        ...
        if comp_id in compartments:
            raise MalformedCompartment(f"duplicate compartment {comp_id!r}")
        close = body.find("[/private]", match.end())
        if close == -1:
            raise MalformedCompartment(f"compartment {comp_id!r} is never closed")
        inner = body[match.end():close]
        if _MARKER_OPEN.search(inner):
            raise MalformedCompartment(f"nested marker inside compartment {comp_id!r}")
        if not inner.strip():
            raise MalformedCompartment(f"compartment {comp_id!r} is empty")
