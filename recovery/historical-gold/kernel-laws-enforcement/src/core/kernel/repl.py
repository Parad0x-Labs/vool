"""A terminal chat where a local model owns every judgment and the kernel enforces the laws.
    ...
    cd /Users/operator/vool/worktrees/exp-kernel-laws-618ea934
    VOOL_HOME=/tmp/vool-kernel-repl-home PYTHONPATH=. \
        /Users/operator/Desktop/vool-local-product/.venv/bin/python -m core.kernel.repl
...
def run_turn(
      ...
      try:
        ...
        for row in data.get("claims", []):
            ...
            if ctype == "observed" and ref in ("user", "conversation") and not has_quantity(text):
                ...
                continue
            if any(c.text == text for c in claims):
                # Exact-duplicate sentences citing different receipts carry zero information
                # (three identical weather pointers, measured live). Disagreement between
                # sources is different TEXT; sameness is redundancy, collapsed here.
                out.append(f"  ! duplicate claim collapsed: {text[:70]}")
                continue
    ...
    for _ob in obligations:
        if _ob.id in _valid_owners or ob_lane.get(_ob.id) != "arithmetic":
            continue
...
def main() -> None:
    ...
    def build_context() -> str:
        ...
        the ledger gives unanswered obligations legal continuity.
        """
        parts: list[str] = []
        if history:
        ...
        if history:
            # TYPED ANSWER ARTIFACT (consensus-5 item 4, minimal by design — no
            # cross-session store, no embeddings): the PREVIOUS answer's lines,
            # numbered, so "the second one" / "keep the first two" resolve to real
            # items instead of echoing (measured: every deictic revision failed).
            prev_lines = [ln.strip() for ln in history[-1][1].splitlines() if ln.strip()][:10]
            if prev_lines:
                numbered = "\n".join(f"  {i}. {ln[:120]}" for i, ln in enumerate(prev_lines, 1))
                parts.append("PREVIOUS ANSWER, ADDRESSABLE BY ITEM NUMBER (when the user says"
