...
def effect_args_hash(effect_id: str, *args: object, **kwargs: object) -> str:
    """Public digest of one effect call's arguments.

    Tape-forking consumers (counterfactual re-derivation) must identify tape entries
    by the SAME hash replay checks use — computing it independently would duplicate
    the canonicalization authority and let a fork drift from the tape's identity.
    """
    return _args_hash(effect_id, args, dict(kwargs))
