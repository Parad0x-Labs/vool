...
while time.monotonic() < deadline:
    ...
    try:
        ...
        # BEGIN IMMEDIATE takes the database's write lock before this iteration's
        # schema-touching work, so a concurrent first-use migration commit cannot
        # land between this statement's prepare and its step. Measured trigger
        # (runs 35615031134 / 35622837193, SQLite 3.45.1): the bare autocommit
        # CREATE TABLE below surfaced SQLITE_SCHEMA (17, "database schema has
        # changed") once per round when a migration committed mid-statement.
