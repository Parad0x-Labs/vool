...
def _render_ledger_rows(events: list[dict]) -> list[dict]:
    ...
    script = (
        ...
        # pageT/pageTF: the receipt prose localizes through these. Stand-ins mirror the
        # English-fallback path (no VOOLT bundle under node), plural-capable like VOOLFMT.
        "function pageT(key, fallback){ return fallback; }\n"
        "function pageTF(key, fallback, params){\n"
        "  var t = String(fallback); var p = params || {};\n"
        "  var m = /\\{(\\w+),\\s*plural,\\s*/.exec(t);\n"
        "  while (m) {\n"
        "    var open = t.indexOf('{', m.index); var depth = 0, end = -1;\n"
        "    for (var i = open; i < t.length; i++) { if (t[i] === '{') depth++;"
        " else if (t[i] === '}') { depth--; if (!depth) { end = i + 1; break; } } }\n"
        "    var body = t.slice(m.index + m[0].length, end - 1);\n"
        "    function branch(name){ var re = new RegExp('\\\\b' + name + '\\\\s*\\\\{').exec(body);"
        " if (!re) return '';\n"
        "      var s = body.indexOf('{', re.index); var d = 0;"
        " for (var j = s; j < body.length; j++) { if (body[j] === '{') d++;"
        " else if (body[j] === '}') { d--; if (!d) return body.slice(s + 1, j); } }\n"
        "      return ''; }\n"
        "    var n = parseInt(p[m[1]], 10); if (isNaN(n)) n = 2;\n"
        "    t = t.slice(0, m.index) + (n === 1 ? branch('one') : branch('other')) + t.slice(end);\n"
        "    m = /\\{(\\w+),\\s*plural,\\s*/.exec(t);\n"
        "  }\n"
        "  Object.keys(p).forEach(function(k){ t = t.split('{' + k + '}').join(String(p[k])); });\n"
        "  return t;\n"
        "}\n"
        + body
